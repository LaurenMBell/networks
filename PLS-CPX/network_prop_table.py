...
def filter_thresholds(ind, fdr, df):
    ...
    filtered_df = df[df["Pooled FDR"] <= fdr]
...
def nodes(df):
    ...
    unique_mets = set(df["cpx_gene"]).union(set(df["pls_metabolite"]))
...
def edges(df):
    ...
    non_nan_count = df[to_check].notna().sum(axis=1) 
    sign_sum = np.nansum(np.sign(df[to_check].values), axis=1)  

    pos_edges = (sign_sum == non_nan_count).sum()  
...
def table_making(file, name):
    ...
    for i in ind_p:
        for j in fdr_p:
            ...
            for col in ["cpx_gene","pls_metabolite","VECPAC r","VECPAC p-values","VECPAC n",
...
df = pd.read_csv("PLS-CPX_FDR.csv")
