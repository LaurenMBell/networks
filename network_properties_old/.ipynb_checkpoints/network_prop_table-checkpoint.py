...
def filter_thresholds(ind, fdr, df):
    ...
    max_p_val = np.nanmax(df[["VECPAC p-values", "LPS p-values", "DSS p-values"]].values, axis=1)
    filtered_df = df[max_p_val <= ind]
...
df_median_without_zeros = pd.read_csv("/Users/laurenbell/Desktop/metabolomics_data/network_properties/median_norm_drop_zero_data/median_wo_fdr_table.csv")
