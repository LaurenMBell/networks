...
def filter_thresholds(ind, fdr, df):
    ...
    max_p_val = filtered_df[["VECPAC_p", "DSS_p", "LPS_p"]].max(axis=1)
...
def nodes(df):
    ...
    unique_mets = set(df["n1"]).union(set(df["n2"]))
...
def table_making(file, name):
    ...
    for i in ind_p:
        for j in fdr_p:
            ...
            for col in ["VECPAC_r", "VECPAC_p", "DSS_r", "DSS_p", "LPS_r", "LPS_p", "FDR"]:
