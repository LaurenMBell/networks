...
#lauren cites AI for help with this step
df = edges[["n1", "n2", "VECPAC_r", "LPS_r", "DSS_r", "Pooled_r", "Sign"]].merge(
    pred_signs[["n1", "n2", "predicted_sign"]],
    on=["n1", "n2"], how="left")
...
df["model_agreement_pooled_sign"] = (df["model_sign"] == df["pooled_r_sign"]
                                     ).map({True: 1, False: -1}, na_action='ignore')

#IF MODEL SIGN == PRED SIGN
df["model_agreement_pred_sign"] = (df["model_sign"] == df["predicted_sign"]
                                   ).map({True: 1, False: -1}, na_action='ignore')

#IF POOLED R SIGN = PRED SIGN
df["pooled_pred_agreement_sign"] = (df["pooled_r_sign"] == df["predicted_sign"]
                                    ).map({True: 1, False: -1}, na_action='ignore')
